...
router.post('/add/b2b-employee', (req, res) => {
    const { name, ph_num, email } = req.body;
    ...
    db.query(checkQuery, [ph_num, email], (checkErr, checkResult) => {
        ...
        const insertQuery = 'INSERT INTO b2b_employee (name, ph_num,permission,email,role) VALUES (?,?,?,?,?)';

        // bcrypt.hash(password.toString(), salt, (err, hash) => {
        db.query(insertQuery, [name, ph_num, 'Pending', email,'b2b_employee'], (err, result) => {
            if (err) {
                console.log(err)
