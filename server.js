...
app.get('/search', async (req, res) => {
    ...
    try {
        ...
        const allLabTestsQuery = 'select Test_id as id,Test_Name as name from laboratory_tests_details;';
        ...
        const alldoctorTestsQuery = 'select id as id,doc_name as name,specializes as name from doctors_details;';
...
app.get('/profile/orders', async (req, res) => {
    if (req.session.user) {
        const user_id = req.session.user.id
        const query = "SELECT  product_name , total_amount ,product.product_id, orders.id,discount ,expected_delivery_date,order_items.quantity,productImageId FROM product INNER JOIN order_items ON product.product_id = order_items.product_id INNER JOIN orders ON orders.id = order_items.order_id INNER JOIN payments ON payments.order_id = orders.id  where orders.user_id = ?;";
        // console.log(user_id)
        const productResults = await new Promise((resolve, reject) => {
            db.query(query, [user_id], (err, result) => {
                if (err) {
